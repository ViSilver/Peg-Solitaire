...
class Table(object):
	...
	def clearBoard(self):
		self.grid = [list(range(self.TableHeight)) for x in range(self.
			TableWidth)]
		self.grid = [list(map(lambda x: Cell(CellType.LivingCell),
								row)) for row in self.grid]


	def selectCellAt(self, pos):
		if self.cellAt(pos).cellType == CellType.Wall or self.cellAt(pos).\
	...
	def deselectCell(self):
		...
		self.setCellAt((self.moves.curX, self.moves.curY), Cell(CellType.
													LivingCell))
		self.isSelected = False
	...
	def getMoves(self, pos):
		...
		if (pos[0] > 1 and self.cellAt((pos[0] - 1, pos[1])).cellType is
			CellType.LivingCell and self.cellAt((pos[0] - 2, pos[1])).
			cellType is CellType.EmptyCell):
			moves.append(Move(pos, (pos[0] - 2, pos[1])))
		# Checking for a move to 'west'
		if (pos[0] < self.TableWidth - 2 and self.cellAt((pos[0] + 1,
						pos[1])).cellType is CellType.LivingCell and
			self.cellAt((pos[0] + 2, pos[1])).cellType is CellType.EmptyCell):
			moves.append(Move(pos, (pos[0] + 2, pos[1])))
		# Checking for a move to 'north'
		if (pos[1] > 1 and self.cellAt((pos[0], pos[1] - 1)).cellType is
			CellType.LivingCell and
			self.cellAt((pos[0], pos[1] - 2)).cellType is CellType.EmptyCell):
			moves.append(Move(pos, (pos[0], pos[1] - 2)))
		# Checking for a move to 'south'
		if (pos[1] < self.TableHeight - 2 and self.cellAt((pos[0],
			pos[1] + 1)).cellType is CellType.LivingCell and
			self.cellAt((pos[0], pos[1] + 2)).cellType is CellType.EmptyCell):
			moves.append(Move(pos, (pos[0], pos[1] + 2)))
